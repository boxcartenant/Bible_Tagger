...
def makeDB(sqlite_database):
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_verse_group_group_id ON verse_group (verse_group_id)')
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_verse_group_verse_id ON verse_group (verse_id)')
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_verse_book_chapter_verse ON verse (book, chapter, verse)')
...
def get_db_stuff(database_file, x_type, y_type, y_value):
    ...
    if y_type == "verse": 
        ...
        # get all verse_group_id's for a given note_id
        # probably overkill as there should only ever be one note per verse_group_id
        query_string = '''
            SELECT DISTINCT vg.verse_group_id
            FROM verse_group vg
            JOIN verse_group_note vgn ON vg.verse_group_id = vgn.verse_group_id
            JOIN note n on vgn.note_id = n.note_id
            WHERE n.note_id = ?
        '''
    else:
